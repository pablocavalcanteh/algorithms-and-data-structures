...
class Adjacent:
    ...
    def __init__(self, vertex, cost):
        self.vertex = vertex
        self.cost = cost
        self.a_star_distance = vertex.goal_distance + self.cost
    ...
    if self.last_position == -1:
      ...
      for i in range(self.last_position + 1):
        print(i, ' - ', self.values[i].vertex.label, ' - ',
         self.values[i].cost, ' - ', self.values[i].vertex.goal_distance, ' - ', self.values[i].a_star_distance)
...
if __name__ == "__main__":
  ...
  a_star_search = AStarSearch(graph.bucharest)
